import os
from .args import FORCE_METADATA_RELOAD, UserFeedback, fail_early
...
class ASDFHandler(UserFeedback):
    ...
    def _get_asdf_dataset(self):
        try:
            ...
            raise Exception(
                "Failed to create ASDF file at '{}': {}.".format(self.asdf_path, e)
            )
        return ds
            ...
            for mseed_file in mseed_file_list:
                ...
                with self._get_asdf_dataset() as ds:
                    ...
                    for trace in st:
                        ...
                        self._log.default("Add '{}' to ASDF.".format(trace))
                        ds.append_waveforms(trace, tag="raw_recording")
                        ...
                        db.execute(
                            ...
                            "WHERE filename='{1}'".format(self.asdf_path, mseed_file)
                        )
        ...
        with self._get_asdf_dataset() as ds:
            ...
            for tag, rows in grouped_rows.items():
                if tag not in ds.waveforms:
                    ...
                    self._log.error(
                        "Skipping station '{0}'. Found station "
                ...
                for row in rows:
                    ...
                    if (
                        self._config.arg(FORCE_METADATA_RELOAD) is True
                        ...
                        station_xml = source.download_stationxml(
                            row.network,
                            row.station,
                            row.location,
                            row.channel,
                        ...
                        self._log.default(
                            "Skip ASDF metadata load for "
                            "'{}.{}.{}.{} | {} - {}'. "
                            "Metadata was already loaded.".format(
                                row.network,
                                row.station,
                                row.location,
                                row.channel,
