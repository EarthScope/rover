...
class Ingester(SqliteSupport, DirectoryScanner):
    ...
    def process(self, temp_file):
        ...
        try:
            run(
                "%s -sqlite %s %s" % (self._mseed_cmd, self._db_path, temp_file),
                self._log,
            )
    ...
    def _copy_all_rows(self, temp_file, rows):
        ...
        with open(temp_file, "rb") as input_file:
            offset = 0
            for row in rows:
                offset, dest = self._copy_single_row(
                    offset, input_file, temp_file, *row
                )
                updated.add(dest)
        return updated

    def _copy_single_row(
        self,
        offset,
        input_buffer,
        temp_file,
        ...
        self._assert_single_day(
            temp_file, starttime, endtime, "%s_%s" % (network, station)
        )
        if offset < byteoffset:
            self._log.warn(
                "Non-contiguous bytes in %s - skipping %d bytes"
                % (temp_file, byteoffset - offset)
            )
            input_buffer.seek(byteoffset - offset, 1)
            offset = byteoffset
        elif offset > byteoffset:
            raise Exception(
                "Overlapping blocks in %s, index is inconsistent regarding byte ranges)"
                % temp_file
            )
        ...
        self._log.debug(
            "Appending %d bytes from %s at offset %d to %s"
            % (raw_bytes, temp_file, byteoffset, dest)
        )
    ...
    def _make_destination(self, network, station, starttime):
        ...
        return join(
            ...
            str(year),
            "%03d" % day,
            "%s.%s.%04d.%03d" % (station, network, year, day),
        )
    ...
    def _assert_single_day(self, temp_file, starttime, endtime, sid):
        ...
        if starttime[:10] != endtime[:10]:
            raise Exception(
                "File %s contains data from more than one day (%s-%s) for %s"
                % (temp_file, starttime, endtime, sid)
            )
