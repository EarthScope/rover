...
class Indexer(ModifiedScanner, DirectoryScanner):
    ...
    def process(self, path):
        ...
        self._workers.execute(
            "%s %s -sqlite %s %s"
            % (
                self._mseed_cmd,
                "-v -v" if self._verbose else "",
                self._timeseries_db,
                path,
            )
        )
...
"""
    ...
    def __init__(self, config):
        ...
        self._single_constraints = {START: None, END: None}
...
""")
    ...
    def _check_database(self):
        try:
            ...
            raise Exception(
                """Cannot access the index table in the database (%s).  Bad configuration or no data indexed?"""
                % self._timeseries_db
            )
    ...
    def _parse_args(self, args):
        ...
        if self._flags[JOIN_QSR] and (
            self._multiple_constraints[SAMPLERATE]
            or self._multiple_constraints[QUALITY]
        ):
            raise Exception(
                "Cannot specify %s / %s AND %s" % (QUALITY, SAMPLERATE, JOIN_QSR)
            )
    ...
    def _assert_unset_flags(self, name1):
        for name2 in self._flags:
            if self._flags[name2]:
                raise Exception(
                    "Cannot specify multiple keys (%s, %s)" % (name1, name2)
                )
    ...
    def _set_snclq(self, snclq):
        ...
        if len(components) < 2 or len(components) > 5:
            raise Exception(
                'Cannot parse %s (expect 2 to 5 values separated by "_" for N_S_L_C_Q)'
                % snclq
            )
        self._set_name_value(NETWORK, components[0])
        self._set_name_value(STATION, components[1])
        if len(components) > 2:
            self._set_name_value(LOCATION, components[2])
        if len(components) > 3:
            self._set_name_value(CHANNEL, components[3])
        if len(components) > 4:
            self._set_name_value(QUALITY, components[4])
    ...
    def _set_time_limit(self, name, value):
        ...
        if (
            self._single_constraints[START]
            and self._single_constraints[END]
            and self._single_constraints[START] > self._single_constraints[END]
        ):
            raise Exception(
                "start time (%s) must be after end time (%s)"
    ...
    def _rows(self, sql, params, stdout):
        ...
        def callback(row):
            if self._flags[JOIN_QSR]:
                ...
                n, s, l, c, ts, r, q = row
                builder.add_timespans(
                    "%s_%s_%s_%s_%s (%g Hz)" % (n, s, l, c, q, r), ts, r
                )
