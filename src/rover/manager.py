...
from .utils import (
    ...
    windows,
    diagnose_error,
    format_year_day_epoch,
)
from .workers import Workers

"""
The core logic for scheduling multiple downloads.  Called by `rover retrieve`.
"""
...
class Chunks:
    ...
    def sncl_ok(self, sncl):
        nslc = sncl.split("_")
        return not self.__chunks or (
            nslc[0] == self.__network and nslc[1] == self.__station
        )
    ...
    def pop(self, progress):
        ...
        description = "%s_%s %s" % (
            self.__network,
            self.__station,
            format_year_day_epoch(right - 24 * 3600),
        )
        ...
        with open(path, "w") as out:
            for sncl, start, end in data:
                ...
                print(
                    "%s %s %s"
                    % (self.format_sncl(sncl), format_epoch(start), format_epoch(end)),
                    file=out,
                )
...
class Retrieval:
    ...
    def __init__(
        self, log, name, temp_dir, delete_files, dataselect_url, force_failures
    ):
        self._log = log
        self._name = name
    ...
    def add_coverage(self, coverage):
        ...
        if (
            coverage
        ):  # this avoids us showing more stations to download than are downloaded
    ...
    def _build_url(self, sncl, start, end):
        url_params = "net=%s&sta=%s&loc=%s&cha=%s" % tuple(
            code if code else "--" for code in tuple(sncl.split("_"))
        )
        return "%s?%s&start=%s&end=%s" % (
    ...
    def _worker_callback(self, command, return_code, path, **kwargs):
        ...
        if return_code:
            ...
            if (
                return_code != ABORT_CODE
            ):  # hide message on ctrl-C as we will exit as well
                self._log.error(
                    "Download %s failed (return code %d)" % (self._name, return_code)
                )
    ...
    def new_worker(self, workers, config_path, rover_cmd):
        ...
        if randint(1, 100) <= self._force_failures:
            self._log.warn(
                "Random failure expected (%s %d)"
                % (mm(FORCEFAILURES), self._force_failures)
            )
            ...
            if windows():
                command = 'pythonw -m rover -f %s %s "%s"' % (
                    config_path,
                    DOWNLOAD,
                    path,
                )
        ...
        callback_function = lambda cmd, rtn, **kwargs: self._worker_callback(
            cmd, rtn, path, **kwargs
        )
...
class Source(SqliteSupport):
    ...
    def _is_complete_initial_reads(self, retry_possible):
        ...
        if self._retrieval.errors.errors:
            ...
            if retry_possible:
                self._log.default(
                    (
                        "Retrieval attempt %d of %d completed with %d errors. "
                        + "We will retry to check that all data were retrieved"
                    )
                    % (
                        self.n_retries,
                        self.download_retries,
                        self._retrieval.errors.errors,
                    )
                )
            ...
            # otherwise, we can't retry so we're done, but failed.
                ...
                raise ManagerException(
                    "Retrieval attempt %d of %d had %d errors on the final attempt."
                    % (
                        self.n_retries,
                        self.download_retries,
                        self._retrieval.errors.errors,
        ...
        # If these boolean conditions are maintained then rover will continue to make download attmepts.
            ...
            if self.download_retries == 1:
                if retry_possible:
                    self._log.default(
                        (
                            "Retrieval attempt %d of %d had no errors and we downloaded data."
                ...
                if (
                ...
                ):  # Determines if data was collected.
                    if (
                        ProgressStatistics.download_retry_count < 100
                    ):  # A hard limit on the amount of times we will attempt to download a request
                        self._log.default(
                            "Successful retrieval, downloaded data so resetting retry count and verify."
                        )
                        ...
                        self._log.default(
                            (
                                "The retrieval has been attempted %d times. ROVER is exiting. Please check the log files and try again later."
                            )
                    ...
                    self._log.default(
                        (
                            "Successful retrieval attempt %d of %d, but no new data downloaded so try again."
                        )
                ...
                # if not, we're going to say we're complete anyway, since we didn't have any errors.
                    ...
                    self._log.default(
                        (
                            "The latest retrieval attempt had no errors.  Stopping at %d retry attempts."
                        )
                        % (self.n_retries)
                    )
                    self._expect_empty = True
        ...
        # When escaping is_complete_initial_reads an additional _new_retrieval should not be run.
            ...
            if self.n_retries == 1:
                ...
                if retry_possible:
                    self._log.default(
                        "The initial retrieval attempt resulted in no errors or data downloaded, will verify."
                    )
                    self._expect_empty = True
                    self._new_retrieval(True)
                ...
                # if not, we're going to say we're complete anyway.
                    ...
                    self._log.default(
                        "The initial retrieval attempt had no errors and no retries are configured."
                    )
                    return True
            # something odd has happened - no data when it wasn't expected
                ...
                if retry_possible:
                    self._log.default(
                        (
                            "Retrieval attempt %d of %d had no errors and downloaded no data. "
                            + "We will check that all data were retrieved."
                        )
                        % (self.n_retries, self.download_retries)
                    )
                    self._expect_empty = True
                    ...
                    raise ManagerException(
                        (
                            "The final retrieval, attempt %d of %d, downloaded no data"
    ...
    def _is_complete_final_read(self, retry_possible):
        ...
        if self._retrieval.errors.errors:
            ...
            if retry_possible:
                self._log.default(
                    (
                        "The latest retrieval attempt had %d errors after %d attempts.  "
            ...
            # otherwise, we can't retry so we're done, but failed.
                ...
                raise ManagerException(
                    "The latest retrieval attempt had %d errors on final attempt (%d of %d)."
                    % (
                        self._retrieval.errors.errors,
                        self.n_retries,
                        self.download_retries,
        ...
        # no errors last retrieval, but we did download some more data (again, unexpected)
            ...
            if self.n_retries == 2:
                if retry_possible:
                    self._log.default(
                        (
                            "The latest retrieval attempt downloaded additional data - "
                ...
                if retry_possible:
                    self._log.default(
                        (
                            "The latest retrieval attempt downloaded unexpected data so trying again "
                            + "to check behavior."
                        )
                    )
                    self._new_retrieval(True)
                ...
                # something odd is happening
                    ...
                    raise ManagerException(
                        (
                            "The previous %d retrieval attempts could not download some "
        ...
        # no errors and no data
            ...
            self._log.default(
                "The final %sretrieval, attempt %d of %d, made no downloads and had no errors, we are complete."
    ...
    def _new_retrieval(self, fetch):
        ...
        if fetch:
            self._log.default(
                "Trying new %sretrieval attempt %d of %d."
                % (self._name, self.n_retries, self.download_retries)
            )
        ...
        if fetch and not self._retrieval.has_chunks():
            self._log.default(
                "%sRetrieval attempt %d of %d is complete."
                % (self._name, self.n_retries, self.download_retries)
            )
    ...
    def _get_availability(self, request, availability_url):
        ...
        response, check_status = post_to_file(
            availability_url,
            request,
            response,
            self._http_timeout,
    ...
    def _parse_line(self, line):
        try:
            n, s, l, c, b, e = (
                "" if token == "--" else token for token in line.split()
            )
            return "%s_%s_%s_%s" % (n, s, l, c), parse_epoch(b), parse_epoch(e)
        except Exception:
            raise Exception(
                'Could not parse "%s" in the response from the availability service'
                % line
            )
    ...
    def _parse_availability(self, response):
        try:
            if response is not None:  # None when no data returned
                sort_file_inplace(
                    self._log, response, self._temp_dir, self._sort_in_python
                )
            ...
            diagnose_error(
                self._log,
                "Problems parsing the availability service response.",
                self._request_path,
                response,
            )
            raise
    ...
    def _scan_index(self, sncl):
        availability = SingleSNCLBuilder(
            self._log, self._timespan_tol, self._timespan_inc, sncl
        )
        ...
        try:
            ...
            self.foreachrow(
                """SELECT coalesce(timespans, '<' || starttime || ' ' || endtime || '>'), samplerate
                                    FROM tsindex
...
class DownloadManager(SqliteSupport):
    ...
    The config_file is overwritten (in temp_dir) because only a singleton should ever exist.
    No attempt is made to delete the file on exit.
    ...
    def __init__(self, config, config_file=None):
        ...
        if config_file:
            ...
            log_verbosity = (
                config.arg(LOGVERBOSITY)
                if config.arg(DEV)
                else min(config.arg(LOGVERBOSITY), 3)
            )
            self._config_path = write_config(
                config, config_file, log_unique=log_unique, log_verbosity=log_verbosity
            )
            if config.arg(WEB):
                self._start_web()
        else:
        ...
        for name in self._sources.keys():
            ...
            for coverage in coverages:
                ...
                for start, end in coverage.timespans:
                    print(
                        "    %s - %s  (%4.2f sec)"
                        % (format_epoch(start), format_epoch(end), end - start)
                    )
            if name != DEFAULT_NAME:
                if source_sncls:
                    print()
                print(
                    "  %s: %d N_S_L_C; %4.2f sec" % (name, source_sncls, source_seconds)
                )
                print()
    ...
    def step(self, quiet=True):
        ...
        if not self._config_path:
            raise Exception(
                "DownloadManager was created only to display data (no config_path)"
            )
        ...
        while self._workers.has_space() and self._has_data():
            ...
            sources = list(
                map(lambda name: self._source(name), sorted(self._sources.keys()))
            )
            while True:
            ...
            for source in self._sources.values():
                ...
                self._db.execute(
                    ...
                    (
                        source.name,
                        progress.stations[1],
                        progress.stations[1] - progress.stations[0],
                        progress.seconds[1],
                        max(0, int(progress.seconds[1] - progress.seconds[0])),
                        source.n_retries,
                        source.download_retries,
                    ),
                )

    def _start_web(self):
        if windows():
            cmd = "pythonw -m rover %s -f %s %s 0" % (
                WEB,
                self._config_path,
                mm(VERBOSITY),
            )
