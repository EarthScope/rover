...
class SummaryLister(SqliteSupport):
    ...
    def _check_database(self):
        try:
            ...
            raise Exception(
                """Cannot access the summary table in the database (%s).  Bad configuration or summary not generated?"""
                % self._timeseries_db
            )
    ...
    def _set_snclq(self, snclq):
        ...
        if len(components) < 2 or len(components) > 4:
            raise Exception(
                'Cannot parse %s (expect 2 to 4 values separated by "_" for N_S_L_C)'
                % snclq
            )
    ...
    def _build_query(self):
        sql, params = (
            "SELECT network, station, location, channel, earliest, latest FROM tsindex_summary ",
            [],
        )
        constrained = False
