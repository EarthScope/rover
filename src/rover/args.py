...
class Arguments(ArgumentParser):
    ...
    * have default config be self-documenting and discoverable
    ...
    def __init__(self):
        super().__init__(
            ...
            epilog=dedent(
                """
                         ...
                         Type "rover help" for more information on available commands."""
                % DEFAULT_FILE
            ),
        )
        ...
        operation_group.add_argument(
            m(H), mm(FULLHELP), action=FullHelpAction, help="show full help details"
        )
        operation_group.add_argument(
            m(BIG_V), mm(VERSION), action="version", version="ROVER %s" % __version__
        )
        operation_group.add_argument(
            mm(FULLCONFIG),
            default=False,
            action="store_bool",
            help="initialize with full configuration file",
            metavar="",
        )
        operation_group.add_argument(
            m(F), mm(FILE), default=DEFAULT_FILE, help="specify configuration file"
        )
        ...
        operation_group.add_argument(
            ...
            default=False,
            action="store_bool",
            help="development mode (show exceptions)?",
            metavar="",
        )
        operation_group.add_argument(
            mm(DELETEFILES),
        ...
        repository_group.add_argument(
            ...
            action="store",
            help="the data directory - data, timeseries.sqlite",
            metavar=DIRVAR,
        )
        ...
        retrieve_group.add_argument(
            mm(TIMESPANINC),
            default=DEFAULT_TIMESPANINC,
            action="store",
            help="fractional increment for starting next timespan",
            metavar=SAMPLESVAR,
            type=float,
        )
        retrieve_group.add_argument(
            mm(TIMESPANTOL),
            default=DEFAULT_TIMESPANTOL,
            action="store",
            help="fractional tolerance for overlapping timespans",
            metavar=SAMPLESVAR,
            type=float,
        )
        retrieve_group.add_argument(
            mm(DOWNLOADRETRIES),
            default=DEFAULT_DOWNLOADRETRIES,
            action="store",
            help="maximum number of attempts to download data",
            metavar=NVAR,
            type=int,
        )
        retrieve_group.add_argument(
            mm(DOWNLOADWORKERS),
            default=DEFAULT_DOWNLOADWORKERS,
            action="store",
            help="number of download instances to run",
            metavar=NVAR,
            type=int,
        )
        retrieve_group.add_argument(
            mm(ROVERCMD),
            default=DEFAULT_ROVERCMD,
            action="store",
            help="command to run rover",
            metavar=CMDVAR,
        )
        retrieve_group.add_argument(
            mm(PREINDEX),
        ...
        retrieve_group.add_argument(
            ...
            default=True,
            action="store_bool",
            help="call ingest after retrieval?",
            metavar="",
        )
        retrieve_group.add_argument(
            mm(INDEX),
        ...
        retrieve_group.add_argument(
            ...
            default=True,
            action="store_bool",
            help="call summary after retrieval?",
            metavar="",
        )
        retrieve_group.add_argument(
            mm(OUTPUT_FORMAT),
            ...
            action="store",
            help='output data format. Choose from "mseed" (miniSEED) or "asdf" (ASDF)',
            metavar="",
        )
        retrieve_group.add_argument(
            mm(ASDF_FILENAME),
            default=DEFAULT_ASDF_FILENAME,
            action="store",
            help="name of ASDF file when ASDF output is specified",
            metavar="",
        )

        # metadata retrieval
        retrieve_md_group = self.add_argument_group("retrieve-metadata arguments")
        retrieve_md_group.add_argument(
            mm(STATIONURL),
            default=DEFAULT_STATIONURL,
            action="store",
            help="station service url",
            metavar=URLVAR,
        )
        retrieve_md_group.add_argument(
            mm(FORCE_METADATA_RELOAD),
            default=False,
            action="store_bool",
            help="force reload of metadata",
            metavar="",
        )
        ...
        download_group.add_argument(
            mm(AVAILABILITYURL),
            default=DEFAULT_AVAILABILITYURL,
            action="store",
            help="availability service url",
            metavar=URLVAR,
        )
        download_group.add_argument(
            mm(DATASELECTURL),
            default=DEFAULT_DATASELECTURL,
            action="store",
            help="dataselect service url",
            metavar=URLVAR,
        )
        download_group.add_argument(
            mm(TEMPDIR),
            ...
            action="store",
            help="temporary storage for downloads",
            metavar=DIRVAR,
        )
        download_group.add_argument(
            mm(TEMPEXPIRE),
            default=DEFAULT_TEMPEXPIRE,
            action="store",
            help="number of days before deleting temp files",
            metavar=DAYSVAR,
            type=int,
        )
        download_group.add_argument(
            mm(HTTPTIMEOUT),
            default=DEFAULT_HTTPTIMEOUT,
            action="store",
        ...
        download_group.add_argument(
            mm(HTTPRETRIES),
            default=DEFAULT_HTTPRETRIES,
            action="store",
            help="max retries for HTTP requests",
            metavar=NVAR,
            type=int,
        )
        download_group.add_argument(
            mm(FORCEFAILURES),
            default=DEFAULT_FORCEFAILURES,
            action="store",
            help="force failures for testing (dangerous)",
            metavar=PERCENTVAR,
            type=int,
        )
        download_group.add_argument(
        ...
        index_group.add_argument(
            ...
            default=False,
            action="store_bool",
            help="process all files (not just modified)?",
            metavar="",
        )
        index_group.add_argument(
            mm(RECURSE),
            default=True,
            action="store_bool",
            help="when given a directory, process children?",
            metavar="",
        )
        ...
        logging_group.add_argument(
            ...
            default=False,
            action="store_bool",
            help="unique log names (with PIDs)?",
            metavar="",
        )
        logging_group.add_argument(
            mm(LOGUNIQUEEXPIRE),
            default=DEFAULT_LOGUNIQUE_EXPIRE,
            action="store",
            help="number of days before deleting unique logs",
            metavar=DAYSVAR,
            type=int,
        )
        logging_group.add_argument(
            mm(LOGVERBOSITY),
            default=DEFAULT_LOGVERBOSITY,
            action="store",
            help="log verbosity (0-6)",
            metavar=NVAR,
            type=int,
        )
        logging_group.add_argument(
            mm(LOGSIZE),
            default=DEFAULT_LOGSIZE,
            action="store",
            help="maximum log size (e.g. 10M)",
            metavar=SIZE,
        )
        logging_group.add_argument(
            mm(LOGCOUNT),
            default=DEFAULT_LOGCOUNT,
            action="store",
        ...
        mseedindex_group.add_argument(
            mm(MSEEDINDEXCMD),
            default=DEFAULT_MSEEDINDEXCMD,
            action="store",
            help="mseedindex command",
            metavar=CMDVAR,
        )
        mseedindex_group.add_argument(
            mm(MSEEDINDEXWORKERS),
            default=DEFAULT_MSEEDINDEXWORKERS,
            action="store",
            help="number of mseedindex instances to run",
            metavar=NVAR,
            type=int,
        )
        ...
        user_feedback_group.add_argument(
            mm(WEB),
            default=True,
            action="store_bool",
            help="auto-start the download progress web server?",
            metavar="",
        )
        user_feedback_group.add_argument(
            mm(HTTPBINDADDRESS),
            default=DEFAULT_HTTPBINDADDRESS,
            action="store",
        ...
        user_feedback_group.add_argument(
            ...
            default="",
            action="store",
            help="address for completion status",
            metavar=ADDRESSVAR,
        )
        user_feedback_group.add_argument(
            mm(EMAILFROM),
            default=DEFAULT_EMAILFROM,
            action="store",
        ...
        self.add_argument(
            ...
            nargs="?",
            type=cmd_or_alias,
            help='use "help" for further information',
        )
        self.add_argument(
            ARGS, nargs="*", help="command arguments (depend on the command)"
        )
    ...
    def write_config(self, path, args, **kargs):
        ...
        if "WRITE_FULL_CONFIG" in kargs and not kargs["WRITE_FULL_CONFIG"]:
            actions = [
                action
                for action in self._actions
                if unbar(action.dest) in LITTLE_CONFIG
            ]
        else:
...
def fail_early(config):
    ...
    if workers > 10:
        raise Exception(
            "Too many workers - risks overloading data center services (%s %d)"
            % (mm(DOWNLOADWORKERS), workers)
        )
    elif workers > 5:
        config.log.warn(
            "Many workers - data center may refuse service (%s %d)"
            % (mm(DOWNLOADWORKERS), workers)
        )
    if config.arg(OUTPUT_FORMAT).upper() == "ASDF":
        try:
            ...
            raise Exception(
                "Missing required 'pyasdf' python package for 'output-format=asdf'"
            )
...
class UserFeedback:
    ...
    def display_feedback(self):
        ...
        if self._email:
            ...
            self._log.info(
                "No email status (see %s configuration parameter)" % mm(EMAIL)
            )
