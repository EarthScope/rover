...
class MetadataSource(UserFeedback):
    ...
    def download_stationxml(
        self, net_code, sta_code, loc_code, cha_code, starttime, endtime
    ):
        """
        Create station web service post request for fdsnws-station service
        and return StationXML.
        """
        request_path = unique_path(self._temp_dir, TMPDOWNLOAD, str(os.getpid()))
        stationxml_path = unique_path(self._temp_dir, TMPDOWNLOAD, str(os.getpid()))
        ws_request = StringIO()
        ws_request.write("level=response\nincludecomments=false\n")
        ws_request.write(
            "{} {} {} {} {} {}\n".format(
                net_code, sta_code, loc_code, cha_code, starttime, endtime
            )
        )
        with open(request_path, "w") as fd:
            ws_request.seek(0)
            shutil.copyfileobj(ws_request, fd)
        stationxml = self._do_download(self._station_url, request_path, stationxml_path)
        ...
        try:
            with SqliteContext(timeseries_db(self._config), self._log) as db:
                if not self.has_tsindex():
                    raise Exception(
                        "No index - first time using rover?"
                    )
                elif not self.has_tsindex_summary():
                    raise Exception(
                        "No index summary exists in database "
                        "'{}'. Run `rover summary` to create one.".format(
                            timeseries_db(self._config)
                        )
                    )
    ...
    def run(self, arg):
        ...
        try:
            ...
            if not os.path.exists(timeseries_db(self._config)):
                raise Exception(
                    "No timeseries index database exists at {}.".format(
                        timeseries_db(self._config)
                    )
                )
            ...
            self._reporter.send_email(
                "Rover Failure", self._reporter.describe_error(RETRIEVE_METADATA, e)
            )
            raise
