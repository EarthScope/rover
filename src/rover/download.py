...
class Downloader(SqliteSupport):
    ...
    def __init__(self, config):
        ...
        clean_old_files(
            self._temp_dir,
            config.arg(TEMPEXPIRE),
            match_prefixes(TMPDOWNLOAD),
            self._log,
        )
    ...
    def run(self, args):
        ...
        if "://" in in_path_or_url:
            ...
            if "&" not in url:
                self._log.warn(
                    (
                        'The URL provided is "%s" - this does not contain any ampersands.  '
                        + "You may have forgotten to quote the URL (and should expect failure)."
                    )
                    % url
                )
        else:
        ...
        if len(args) == 2:
            ...
            out_path, delete_out = (
                unique_path(self._temp_dir, TMPDOWNLOAD, in_path_or_url),
                True,
            )

        try:
            if self._do_download(
                get, url, in_path, out_path
    ...
    def _do_download(self, get, url, in_path, out_path):
        ...
        if get:
            response, check_status = get_to_file(
                url, out_path, self._http_timeout, self._http_retries, self._log
            )
            check_status()
        else:
            try:
